...
class CartModel {
    ...
    public function getCart($email) {
        $name = $price = $quantity= $image = $product_id = null;
        // Khai báo biến
        ...
        $query = "SELECT c.quantity, p.name, p.price,  ip.image, c.product_id
                  FROM cart AS c
        ...
        $stmt->bind_result($quantity, $name, $price, $image,$product_id );
